...
def _author_uvs(mesh: "trimesh.Trimesh", mesh_prim: "UsdGeom.Mesh") -> bool:
    ...
    try:
        ...
        if len(uv_flipped) == len(mesh.vertices):
            primvars_api = UsdGeom.PrimvarsAPI(mesh_prim.GetPrim())
...
def _save_texture_if_any(mesh: "trimesh.Trimesh", textures_dir: Path, name_hint: str) -> str:
    ...
    # - mat.baseColorTexture (PIL.Image) - primary for glTF/GLB files
    # - mat.image (PIL.Image or np.ndarray) - alternative location
    # - occasionally other texture variants
    candidates = []
...
def _bind_preview_surface_with_texture(
    ...
    tex.CreateInput("st", Sdf.ValueTypeNames.Float2).ConnectToSource(st_reader.CreateOutput("result", Sdf.ValueTypeNames.Float2))
    ...
    ps.CreateInput("diffuseColor", Sdf.ValueTypeNames.Color3f).ConnectToSource(tex.CreateOutput("rgb", Sdf.ValueTypeNames.Color3f))
    ...
    material.CreateSurfaceOutput().ConnectToSource(ps.CreateOutput("surface", Sdf.ValueTypeNames.Token))
    material.CreateDisplacementOutput().ConnectToSource(ps.CreateOutput("displacement", Sdf.ValueTypeNames.Float))
...
def main():
    ...
    # Only process modelo{1..56}.glb files
    glb_files = []
    for i in range(1, 57):
        glb_file = glb_dir / f"modelo{i}.glb"
        if glb_file.exists():
