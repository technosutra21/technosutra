...
function updateIndexHTML() {
    // Dev: console.log('🔄 Atualizando index.html para usar pasta usdz...\n');
    
    // Verificar se o arquivo index.html existe
    if (!fs.existsSync(INDEX_FILE)) {
        // Dev: console.error(`❌ Arquivo não encontrado: ${INDEX_FILE}`);
        return false;
    }
    ...
    if (!fs.existsSync(backupFile)) {
        ...
        // Dev: console.log(`💾 Backup criado: ${backupFile}`);
    }
    ...
    if (oldLoadModelFunction) {
        ...
        // Dev: console.log('✅ Função loadModel atualizada para usar pasta usdz');
    } else {
        // Dev: console.log('⚠️  Função loadModel não encontrada - tentando outro método...');
        ...
        if (content.match(usdzLineRegex)) {
            ...
            // Dev: console.log('✅ Linha usdzSrc atualizada para usar pasta usdz');
        } else {
            // Dev: console.log('⚠️  Não foi possível encontrar a linha usdzSrc para atualizar');
        }
    }
    
    // Verificar se a pasta usdz existe
    if (!fs.existsSync(USDZ_DIR)) {
        // Dev: console.log(`📁 Criando pasta ${USDZ_DIR}...`);
    ...
    // Dev: console.log(`✅ Arquivo atualizado: ${INDEX_FILE}`);
    // Dev: console.log(`📁 Pasta USDZ: ${USDZ_DIR}`);
    // Dev: console.log(`📄 Arquivos USDZ encontrados: ${usdzFiles.length}`);
    
    if (usdzFiles.length > 0) {
        // Dev: console.log('\n📋 Arquivos USDZ disponíveis:');
        usdzFiles.forEach(file => // Dev: console.log(`   - ${file}`));
    } else {
        // Dev: console.log('\n⚠️  Nenhum arquivo USDZ encontrado na pasta. Execute a conversão primeiro:');
        // Dev: console.log('   .\\Convert-GLB-to-USDZ.ps1');
    }
    
    // Dev: console.log('\n🎉 Atualização concluída!');
    // Dev: console.log('🚀 Agora o index.html procurará arquivos USDZ na pasta usdz/');
...
function showDifferences() {
    ...
    if (!fs.existsSync(backupFile)) {
        // Dev: console.log('⚠️  Arquivo de backup não encontrado');
        return;
    }
    ...
    if (originalContent === newContent) {
        // Dev: console.log('ℹ️  Nenhuma alteração detectada');
        return;
    }
    
    // Dev: console.log('\n🔍 Principais alterações:');
    // Dev: console.log('   - Caminho USDZ atualizado de "modelo{ID}.usdz" para "usdz/modelo{ID}.usdz"');
    // Dev: console.log('   - Função loadModel modificada para usar subpasta usdz');
}
...
function restoreBackup() {
    ...
    if (!fs.existsSync(backupFile)) {
        // Dev: console.log('❌ Arquivo de backup não encontrado');
        return false;
    }
    ...
    // Dev: console.log('✅ Backup restaurado com sucesso');
    return true;
}
